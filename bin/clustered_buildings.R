...
required <- c('argparse', 'lattice', 'latticeExtra', 'data.table', 'ape')
...
main <- function(arguments){
  ...
  parser$add_argument(
      '-o', '--outfile', metavar='FILE.pdf ...', default='buildings.pdf', nargs = '*',
  ...
  ## for now....
  annotation <- annotation[annotation$visit == 'baseline', c('specimenid', 'category')]

  by_specimen <- data.table::fread(args$by_specimen, colClasses=c(tax_id='character'))
  by_specimen <- by_specimen[by_specimen$specimen %in% annotation$specimenid,]
  ...
  levels <- names(sort(sapply(with(data, split(abundance, tax_name)), mean), decreasing=TRUE))
  ...
  ff <- barchart(abundance ~ specimen,
                 groups=tax_name,
                 data=data,
                 stack=TRUE,
                 auto.key=list(space='right', reverse.rows=TRUE),
                 axis=axis.grid,
                 par.settings=my.theme(n=12),
                 main=args$title)
  get_device(args$outfile, ff, width=11, height=8.5)
