...
(though some classifications are at a higher rank). Note that the
input file contains otus not represented in this project - we'll
remove these. In addition, pplacer failed to provide a classification
for some of these otus. We'll assign these a rank of
`Superkingdom=Bacteria` for now, but we may eventually provide
classification for some of these or even find that they are actually
not bacteria at all.
...
missing <- sort(otus[!otus %in% mc_concat$otu])
missing
...
Provide missing classifications (for now).

```{r}
for(otu in missing){
  taxlist[[otu]] <- setNames(c('Root', 'Bacteria'), c('Rank1', 'Superkingdom'))
}
tail(taxlist, 4)
```
...
across objects, so we have to perform explicit checks that each object
has the same number of elements, etc.
...
Save output as serialized R objects. Include project status and a few intermediate objects that may be useful.

```{r}
git_status <- system('pwd; git --no-pager log -n1; git status', intern=TRUE)
save(phyloseq, classif, git_status, file=args$rda)
```
