...
- **2016-11-03**: Remove censored environmental sequences (which corresponded to sequences with no classification)
- **2017-05-08**: Update output to include specimens from Manhart R01 project
...
## hack to run interactively for development
if(TRUE){
  arguments <- unlist(strsplit(rev(commandArgs(trailingOnly=TRUE))[1], split=' '))
  save(arguments, file='arguments.rda')
}else{
  load('arguments.rda')
}
...
```{r}
keepcols <- c('sampleid', 'project', 'batch', 'study_id',
...
```{r}
otus <- sort(tre$tip.label)
```

## pplacer classifications

Classifications by OTU; this file contains classifications at a range
of target ranks (from superkingdom to species). Note that the input
file contains otus not represented in this project - we'll remove
these. Confirm that all otus in the tree are included among the
classifications.

```{r}
mc_concat <- read.csv(args$classifications, as.is=TRUE)
mc_concat$otu <- sapply(strsplit(mc_concat$name, split=':'), '[', 1)
mc_concat <- mc_concat[mc_concat$otu %in% otus,]
stopifnot(all(otus %in% mc_concat$otu))

ranks <- c("superkingdom", "phylum", "class", "order", "family", "genus", "species")
classif <- data.frame(otu=otus)
for (rank in ranks){
...
## taxonomic order. This is a useful ordering for certain
## visualizations.
...
Read the OTU table produced by dada2 (`otutab`) and limit to specimens
in the CRC3 project and otus included in the tree. Remove rows for
otus no longer represented.

```{r}
otutab <- read.table(
    args$otu_tab, as.is=TRUE, check.names=FALSE, header=TRUE, sep=',', row.names=1)
otutab <- otutab[otus,colnames(otutab) %in% annotation$sampleid]
otutab <- otutab[rowSums(otutab) > 0,]
dim(otutab)
stopifnot(nrow(otutab) == length(otus))
```

# assemble and save the phyloseq object

Annoyingly, phyloseq does not do us the favor of enforcing consistency
across objects (specimen labels must be consistent, but otus missing
in one object will be silently dropped), so we have to check the
number of otus in `phy`.
...
Confirm that all otus are represented

```{r}
stopifnot(length(setdiff(tre$tip.label, phy_tree(phy)$tip.label)) == 0)
```

Save output as serialized R objects. Include project status and a few
intermediate objects that may be useful.

```{r}
git_status <- system('pwd; git --no-pager log -n1; git status', intern=TRUE)
save(phy, classif, git_status, file=args$rda)
```

```{sh}
md5sum output-crc3/*.rda
```
