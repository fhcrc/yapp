...
specimens (in columns) according to the dendrogram - but before
reordering, confirm that the starting order is identical to the order
of specimen annotations in ``sample_data(phy)``.
...
rownames(plot.data)[1] <- "(other)"

identical(sample_data(phy)$specimen_name, colnames(otutab))
plot.data <- plot.data[,order.dendrogram(specimen.dend)]
```

Prepare specimen annotation. These are not ordered according to the
dendrogram here, because they are placed into dendrogram order in
``dendrogramGrob``

```{r}
sexpartners  <- sample_data(phy)$sexpartners
ngu_status <- sample_data(phy)$ngu_status
```
